...
gender_earnings_gap_1 <- lm(log_ing_m_win ~ Female, data = Data_P4)
...
gender_earnings_gap_2_OLS <- lm(log_ing_m_win ~ Female + Edad_win + Edad2+ tfirma_factor + Trabajo_informal + Independiente + 
...
FWL_resid_1 <- residuals(lm(log_ing_m_win ~Edad_win + Edad2+ tfirma_factor + Trabajo_informal + Independiente + 
                              oficio_factor + edu_factor + Horas_trabajadas_win + Experiencia_win + estrato_factor+dummy_jefe, data = Data_P4)) 
...
stargazer(gender_earnings_gap_1,gender_earnings_gap_2_FWL,type = "text",out = "Punto_4_Brecha_Salarial_Genero.tex",
          ...
          dep.var.labels = c("Ln(Salario Mensual) sin controles","Ln(Salario Mensual) con controles"), # CAMBIO ETIQUETA
...
Female_funcion<-function(Data,Indice){
  # CAMBIO: log_ing_m_win (Mensual)
  res_1 <- residuals(lm(log_ing_m_win ~ Edad_win + Edad2+ tfirma_factor + Trabajo_informal + Independiente + 
...
# CAMBIO: log_ing_m_win (Mensual)
Salario_Edad_Female <- lm(log_ing_m_win ~ Edad_win + Edad2, data = Female_Data) 
...
Female_funcion_2 <-function(Data,Indice){
  # CAMBIO: log_ing_m_win (Mensual)
  Reg_1 <- lm(log_ing_m_win ~ Edad_win + Edad2, data = Data, subset= Indice)
...
Salario_Edad_Male <- lm(log_ing_m_win ~ Edad_win + Edad2, data = Male_Data) 
...
stargazer(Salario_Edad_Female,Salario_Edad_Male,type = "text",out = "Punto_4_Perfil_Salario_Edad_Genero.tex",
          omit = "Constant",                             # Quitar intercepto              
          omit.stat = c("f", "ser", "aic", "bic", "ll","rsq"), # Omitir estadísticas no deseadas
          dep.var.labels = c("Ln(Salario Mensual)"),     # CAMBIO ETIQUETA
...
# CAMBIO: Data_P4$log_ing_m_win y etiquetas
plot(Data_P4$Edad_win, (Data_P4$log_ing_m_win), 
     xlab = "Edad", ylab = "Ln(Salario Mensual)", 
     col = alpha("grey", 0.8), #main = "Perfil Salario-Edad por género", 
     xlim = c(15, 75), ylim = c(10, 16),  # Ajuste rango Y para salario mensual (aprox log(1e6) es 13.8)
     xaxp = c(20, 80, 6), yaxp = c(10, 16, 6))  # Ajuste frecuencia eje Y
...
# Nota: Ajusta la posición 'y' (el 5 original) si queda fuera del nuevo rango (10-16)
text(Max_Male+2, 11, round(Max_Male, 1), col = "red", pos = 4)  # Etiqueta para línea roja
text(Max_Female-2, 11, round(Max_Female, 1), col = "blue", pos = 2)  # Etiqueta para línea azul

# cerrar el pdf
dev.off()
# dev.off() # Nota: Tenías un dev.off() extra al final
