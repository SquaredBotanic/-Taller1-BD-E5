...
#i. Regresión: Log(Ingreso)=b1 + b2(age) + b3(age)^2 + u (sin controles)
# CAMBIO: log_ing_m_win (Mensual)
model_Age_wage <- lm(log_ing_m_win ~ Edad_win + Edad2, data = data_webs) #Realizamos la regresión
summary(model_Age_wage)
stargazer(model_Age_wage, type = "text") # Modelo simple

#ii. Regresión: Log(Ingreso)=b1 + b2(age) + b3(age)^2 + u (con controles)
# CAMBIO: log_ing_m_win (Mensual)
model_Age_wage_cont1 <- lm(log_ing_m_win ~ Edad_win + Edad2 + Mujer + estrato_factor + dummy_jefe + edu_factor + tfirma_factor + Trabajo_informal + Independiente + oficio_factor  + Horas_trabajadas_win + Experiencia_win, data = data_webs) #Realizamos la regresión
...
# CAMBIO: log_ing_m_win (Mensual)
n_model_Age_wage_cont1 <- lm(log_ing_m_win ~ Edad_win + Edad2 + Mujer + estrato_factor + dummy_jefe + edu_factor + tfirma_factor + Trabajo_informal + Independiente + oficio_factor  + Horas_trabajadas_win + Experiencia_win, data = data_webs2) #Realizamos la regresión
...
# CAMBIO: log_ing_m_win (Mensual)
model_Age_wage_cont1_vt <- lm(log_ing_m_win ~ Edad_win + Edad2 + Mujer + estrato_factor + dummy_jefe + edu_factor + tfirma_factor + Trabajo_informal + Independiente + oficio_factor  + Horas_trabajadas_win + Experiencia_win, data = data_webs3) #Realizamos la regresión
...
stargazer(model_Age_wage_cont1,n_model_Age_wage_cont1,model_Age_wage_cont1_vt,
          ...
          align = TRUE, 
          dep.var.labels.include = FALSE,
          dep.var.caption  = "Log(Ingreso Mensual)", # CAMBIO ETIQUETA       
...
# CAMBIO: y = log_ing_m_win
graf1 <- ggplot(data_webs, aes(x = predicted, y = log_ing_m_win))+
  geom_point(color = "grey")+
  geom_abline(color ="red")+
  xlab("Predicción")+
  ylab("Log(Ingreso Mensual)")+ # CAMBIO ETIQUETA
  theme_minimal()
...
# CAMBIO: y = log_ing_m_win
graf2 <- ggplot(data_webs3, aes(x = predicted_cont, y = log_ing_m_win))+
  geom_point(color = "grey")+
  geom_abline(color ="red")+
  xlab("Predicción")+
  ylab("Log(Ingreso Mensual)")+ # CAMBIO ETIQUETA
...
# CAMBIO: data_webs$log_ing_m_win
err <- data_webs$log_ing_m_win - data_webs$predicted
err2 <- err^2
(rmse <-sqrt(mean(err2))) # 0.79

# Calcular error Modelo Completo
# CAMBIO: data_webs3$log_ing_m_win
err_cont <- data_webs3$predicted_cont - data_webs3$log_ing_m_win
...
Age_Wage_Profile_fn<-function(data_webs,index){
  # CAMBIO: log_ing_m_win en la formula
  f <- lm(log_ing_m_win ~ Edad_win + Edad2, data = data_webs, subset= index)
  b1 <- f$coefficients[2]
  b2 <- f$coefficients[3]
  max_edad <- -(b1)/(2*b2)
  return(max_edad)  }

#Verifiquemos que la función... funciona! jeje
...
Hist_mod_simp <- ggplot(Dist_Peak_age_mod_simple, aes(x = V1)) +
  ...
  labs(title = "Panel A: Modelo sin controles",
       x = "Edad",
       y = "Ln(Ingreso Mensual)") + # CAMBIO ETIQUETA
  theme_minimal() +
  theme(
...
#Grafica
# CAMBIO: y = log_ing_m_win
Age_wage_P_plot <- ggplot(data_webs, aes(x = Edad_win, y = log_ing_m_win)) +
  ...
  labs(title = "Panel A:Modelo simple",
       x = "Edad",
       y = "Log(Ingreso Mensual)") + # CAMBIO ETIQUETA
  theme_minimal() +
  theme(
    plot.title = element_text(size = 20, face = "bold"),      # Aumenta el tamaño del título
    axis.title.x = element_text(size = 20),                   # Aumenta el tamaño del label del eje X
...
Age_Wage_Profile_mod_cont<-function(data_webs3,index){
  # CAMBIO: log_ing_m_win en la formula
  f <- lm(log_ing_m_win ~ Edad_win + Edad2 + Mujer + estrato_factor + dummy_jefe + edu_factor + tfirma_factor + Trabajo_informal + Independiente + oficio_factor  + Horas_trabajadas_win + Experiencia_win, data = data_webs3, subset = index)
  b1 <- f$coefficients[2]
  b2 <- f$coefficients[3]
  max_edad <- -(b1)/(2*b2)
  return(max_edad)  }
...
# CAMBIO: y = log_ing_m_win
Age_wage_P_plot_cont <- ggplot(data_webs3, aes(x = Edad_win, y = log_ing_m_win)) +
  geom_point(aes(color = "Real"), alpha = 0.5) +  # Puntos para valores reales
